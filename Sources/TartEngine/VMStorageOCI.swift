...
package class VMStorageOCI: PrunableStorage {
  let baseURL: URL
  ...
  package func exists(_ name: RemoteName) -> Bool {
    VMDirectory(baseURL: vmURL(name)).initialized
  }

  package func digest(_ name: RemoteName) throws -> String {
  ...
  package func open(_ name: RemoteName) throws -> VMDirectory {
  ...
  package func create(_ name: RemoteName, overwrite: Bool = false) throws -> VMDirectory {
    let vmDir = VMDirectory(baseURL: vmURL(name))
  ...
  package func move(_ name: RemoteName, from: VMDirectory) throws{
  ...
  package func delete(_ name: RemoteName) throws {
    try FileManager.default.removeItem(at: vmURL(name))
    try gc()
  }

  package func gc() throws {
  ...
  package func list() throws -> [(String, VMDirectory, Bool)] {
  ...
  package func pull(_ name: RemoteName, registry: Registry, concurrency: UInt, config: any ConfigProtocol) async throws {
    ...
    if !exists(digestName) {
      ...
      if let uncompressedDiskSize = manifest.uncompressedDiskSize() {
        ...
        // TODO: callback to Command not available here
  ...
  package func linked(from: RemoteName, to: RemoteName) -> Bool {
    do {
  ...
  package func link(from: RemoteName, to: RemoteName) throws {
