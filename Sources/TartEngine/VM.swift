...
package struct UnsupportedRestoreImageError: Error {
}

package struct NoMainScreenFoundError: Error {
}

package struct DownloadFailed: Error {
}

package struct UnsupportedOSError: Error, CustomStringConvertible {
  package let description: String

  package init(_ what: String, _ plural: String, _ requires: String = "running macOS 13.0 (Ventura) or newer") {
    description = "error: \(what) \(plural) only supported on hosts \(requires)"
  }
}

package struct UnsupportedArchitectureError: Error {
}

package class VM: NSObject, VZVirtualMachineDelegate, ObservableObject {
  // Virtualization.Framework's virtual machine
  @Published package var virtualMachine: VZVirtualMachine

  // Virtualization.Framework's virtual machine configuration
  package var configuration: VZVirtualMachineConfiguration

  // Semaphore used to communicate with the VZVirtualMachineDelegate
  package var sema = AsyncSemaphore(value: 0)

  // VM's config
  package var name: String

  // VM's config
  package var vmConfig: VMConfig

  package var network: Network

  package init(vmDir: VMDirectory,
  ...
  package static func retrieveIPSW(remoteURL: URL, config: any ConfigProtocol) async throws -> URL {
  ...
  @available(macOS 13, *)
  package static func linux(vmDir: VMDirectory, diskSizeGB: UInt16) async throws -> VM {
    // Create NVRAM
  ...
  package func start(recovery: Bool, resume shouldResume: Bool) async throws {
  ...
  package static func craftConfiguration(
    diskURL: URL,
    nvramURL: URL,
    vmConfig: VMConfig,
    network: Network = NetworkShared(),
  ...
  package func virtualMachine(_ virtualMachine: VZVirtualMachine, networkDevice: VZNetworkDevice, attachmentWasDisconnectedWithError error: Error) {
